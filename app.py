...
def run_officer(statement, course, brutality):
    ...
    task = Task(
        ...
        expected_output=(
            ...
            "Most statements you see score 3-5. Do not inflate. A statement "
            "with no concrete evidence CANNOT score above 3, no matter how "
            "pleasant the writing is.\n\n"
...
if not active:
    ...
    if not st.session_state.get("bingo_clean_shown"):
        st.balloons()
        st.session_state.bingo_clean_shown = True
    st.success("Clean sheet. No bingo squares hit.")
else:
    st.session_state.bingo_clean_shown = False
    if found >= 5:
